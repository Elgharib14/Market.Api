...
{
    public class AuthenticationRep : IAuthentication
    {
        ...
        public AuthenticationRep(UserManager<AppUser> userManager,IOptions<JWT> jwt , RoleManager<IdentityRole> roleManager)
        {
        ...
        public async Task<AuthModell> Registration(RegisterVM model)
        {
            if (await _userManager.FindByEmailAsync(model.Email!) is not null)
                return new AuthModell { Massege = "Email is already Registered" };

            if (await _userManager.FindByNameAsync(model.UserName!) is not null)
                return new AuthModell { Massege = "UserName is already Registered" };
            ...
            if (!result.Succeeded)
            {
                ...
                return new AuthModell { Massege = errors };
            }
        ...
        public async Task<string> AddRole(RoleVM model)
        {
        ...
        private async Task<JwtSecurityToken> CreatejwtToken(AppUser user)
        {
            ...
            var Jwtsecuritytoken = new JwtSecurityToken(
                ...
                claims: claims,
                expires: DateTime.Now.AddDays(jwt.Value.DeyrationInDays),
